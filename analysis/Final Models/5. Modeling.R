...
setwd("~/Documents/Bioinformatics/Classes/FALL/HST.953/Git_Files_Cardiogenic/953-2019-team_i/analysis/Final Models")
...
# NULL values cleaning pre-analytics - this file is ready for analysis with only no predictors with more 25% of NULL. (all were deleted in preformatting) 
eicu <- read_csv("./Dataset/eICU.csv", col_names = TRUE)%>%dplyr::select(-"X1")

# Last formatting to match eICU data with MIMIC
eicu <- eicu%>%dplyr::select(-c("removea", "removeb","patientsunitsstayid"))
eicu <- eicu%>%mutate(
  iabp = replace_na(0),
  impella = replace_na(0))
eicu$age <- ifelse(eicu$age == "> 89", "90", eicu$age)
eicu$gender <- case_when(eicu$gender  == 'Female' ~ "F",
                         eicu$gender  == "Male" ~ "M",
                         TRUE ~ eicu$gender )
eicu$ethnicity <- case_when(eicu$ethnicity  == 'African American' ~ "BLACK",
                         eicu$ethnicity  == "Asian" ~ "ASIAN",
                         eicu$ethnicity  == "Caucasian" ~ "WHITE",
                         eicu$ethnicity  == "Hispanic" ~ "HISPANIC",
                         eicu$ethnicity  == "Hispanic" ~ "HISPANIC",
                         eicu$ethnicity  == "Native American" ~ "OTHER",
                         eicu$ethnicity  == "Other/Unknown" ~ "OTHER",
                         TRUE ~ eicu$ethnicity)

# Selecting shock patients only
eicu <- eicu%>%filter(scai_shock!="NO")
eicu <- eicu%>%dplyr::select(-c("ph_mean", "removeb", "ptt_max", "urineoutput"))
eicu <- eicu%>%dplyr::select(-c("scai_shock", "removea", "patientunitstayid"))
# MIMIC shock patients were already pre-filtered 
mimic <- read_csv("./Dataset/mimic_shock.csv", col_names = TRUE)%>%dplyr::select(-"X1")
...
write.csv(missing_eicu, file="missing_eicu_percentage.csv")
write.csv(missing_mimic, file="missing_mimic_percentage.csv")
...
# Comparing eicu and mimic

compare_df_cols(mimic,eicu)

# Adapting column names
colnames(eicu)[colnames(eicu) == "mean_bp_calc"] <- "mean_bp_mean"

# Removing cardiac arrest

mimic <- mimic%>%dplyr::select(-cardiac_arrest_and_ventricular_fibrillation)
eicu <- eicu%>%dplyr::select(-cardiac_arrest_and_ventricular_fibrillation)

#write.csv(eicu, file="imputed_eicu.csv")
#write.csv(mimic, file="imputed_mimic.csv")
# Weka file

# The folowing allows us once again to identifying the discordant columns between eicu and mimic
...
'chronic_kidney_disease','chronic_obstructive_pulmonary_disease_and_bronchiectasis',
...
eicu <- eicu%>%mutate_at(c("rrt",'vent','iabp','total_pressors'),  #'impella',,'dopamine','dobutamine','norepinephrine','epinephrine','phenyl','vasopressin','milrinone',
                         function(x){fct_recode(as.factor(x),"yes"="1", "no"="0")})
eicu <- eicu%>%mutate_at(c("hospital_mortality","doubled_creat","any_inotrope"), function(x){fct_recode(as.factor(x),"yes"="1", "no"="0")})
eicu <- eicu%>%mutate_at("ethnicity", function(x){as.factor(x)})
eicu$age <- as.numeric(eicu$age)
...
mimic <- mimic%>%mutate_at(c("rrt",'vent','iabp','impella','total_pressors', "doubled_creat", "cabg", "pci", "any_inotrope"), 
                         function(x){fct_recode(as.factor(x),"yes"="1", "no"="0")})
...
# Using only attributes present in both datasets
# Now that attributes converted in appropriate type - can export to Weka to run model
eicu_w <- eicu[, matched]
mimic_w <- mimic[, matched]

foreign::write.arff(eicu_w ,file= "imputed_eicu_w.arff")



eicu_analysis <- eicu_w
mimic_analysis <- mimic_w


# Imputation #2 because not complete 
eicu_m <- mice(eicu_analysis)
eicu_analysis <- complete(eicu_m, 1)
eicu <- eicu_analysis
s <- skim_to_wide(mimic)
#removing iabp because factor of 0 only
eicu <- eicu%>%dplyr::select(-iabp)
...
varImp(logistic_reg_model_f)
plot(varImp(logistic_reg_model_f))
...
predictions_balanced <- predict(logistic_reg_model_f, mimic_analysis)
predictions_balanced <- predict(logistic_reg_model_f, mimic_analysis, type = 'prob')
...
pred_b <- ROCR::prediction(predictions_balanced[,2], mimic_analysis$hospital_mortality)
